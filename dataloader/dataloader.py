# import torch
# from torch.utils.data import DataLoader
# from torch.utils.data import Dataset
# import os
# import numpy as np
# from .augmentations import DataTransform

# class Load_Dataset(Dataset):
#     def __init__(self, dataset, config, training_mode):
#         super(Load_Dataset, self).__init__()
#         self.training_mode = training_mode

#         X_train = dataset["samples"]
#         y_train = dataset["labels"]

#         # 处理维度: 确保是 (Batch, Channels, Length)
#         if len(X_train.shape) < 3:
#             X_train = X_train.unsqueeze(1) # 增加 Channel 维

#         # 确保 Channel 维度在第 2 位 (Index 1)
#         # 假设数据长度通常比通道数大，如果第2维比第3维大，说明可能是 (Batch, Length, Channel)，需要转置
#         if X_train.shape[1] > X_train.shape[2]:
#             X_train = X_train.permute(0, 2, 1)

#         self.x_data = X_train
#         self.y_data = y_train

#         self.len = X_train.shape[0]
        
#         # 只有在自监督训练时才做数据增强
#         if training_mode == "self_supervised":
#             self.aug1, self.aug2 = DataTransform(self.x_data, config)

#     def __getitem__(self, index):
#         if self.training_mode == "self_supervised":
#             return self.x_data[index], self.y_data[index], self.aug1[index], self.aug2[index]
#         else:
#             # 在微调或测试时，不需要增强，只返回原始数据
#             return self.x_data[index], self.y_data[index], self.x_data[index], self.x_data[index]

#     def __len__(self):
#         return self.len

# def get_loso_loaders(full_data_path, subject_id_to_exclude, configs, training_mode):
#     # 加载第一步生成的完整数据
#     full_dataset = torch.load(full_data_path)
    
#     samples = full_dataset["samples"]
#     labels = full_dataset["labels"]
#     subjects = full_dataset["subject_ids"]

#     # === LOSO 核心逻辑 ===
#     # 测试集 = 当前指定的 subject_id
#     # 训练集 = 除了当前 subject_id 之外的所有人
    
#     # 注意：subject_ids 类型可能是 Tensor 也可能是 numpy，转成 numpy 处理比较方便
#     if isinstance(subjects, torch.Tensor):
#         subjects = subjects.numpy()

#     test_indices = np.where(subjects == subject_id_to_exclude)[0]
#     train_indices = np.where(subjects != subject_id_to_exclude)[0]

#     train_data = {
#         "samples": samples[train_indices],
#         "labels": labels[train_indices]
#     }
    
#     test_data = {
#         "samples": samples[test_indices],
#         "labels": labels[test_indices]
#     }

#     # 创建 Dataset
#     train_dataset = Load_Dataset(train_data, configs, training_mode)
#     test_dataset = Load_Dataset(test_data, configs, training_mode)
    
#     # 验证集策略：
#     # 在 LOSO 中，为了严谨，可以从训练集(Train)里再分出一部分做验证(Valid)
#     # 或者简单起见，暂时用 Test 集做 Valid (但不要根据 Valid 调参，否则会泄露)
#     valid_dataset = test_dataset 

#     train_loader = DataLoader(dataset=train_dataset, batch_size=configs.batch_size,
#                               shuffle=True, drop_last=configs.drop_last, num_workers=0)
    
#     valid_loader = DataLoader(dataset=valid_dataset, batch_size=configs.batch_size,
#                               shuffle=False, drop_last=configs.drop_last, num_workers=0)

#     test_loader = DataLoader(dataset=test_dataset, batch_size=configs.batch_size,
#                              shuffle=False, drop_last=False, num_workers=0)

#     return train_loader, valid_loader, test_loader

# # 现在证实，如果使用cnn，那么这么单纯的映射是最优解,这个版本只考虑单通道
import torch
from torch.utils.data import DataLoader, Dataset
import os
import numpy as np
from sklearn.model_selection import train_test_split
from .augmentations import DataTransform, time_domain_to_letters

class SignalTokenizer:
    def __init__(self):
        # === 整数编码映射 ===
        # 将字符映射为不同的"幅度"级别
        # 0 通常用于背景，1-4 用于不同的波形特征
        # 这种阶梯状的信号非常适合 CNN 进行卷积提取
        self.vocab_map = {
            'O': 0.0,
            'A': 1.0,
            'V': 2.0,
            'U': 3.0,
            'D': 4.0
        }
        
        self.default_val = 0.0

    def convert_tokens_to_ids(self, token_array):
        """
        输入: Numpy String Array -> ['A', 'V', 'O'...]
        输出: Torch Float Tensor -> [1.0, 2.0, 0.0...]
        """
        if not isinstance(token_array, np.ndarray):
            token_array = np.array(token_array)

        # 使用 numpy 下标查找，速度最快
        # np.vectorize 将字典 get 方法应用到整个数组
        mapping_func = np.vectorize(lambda x: self.vocab_map.get(x, self.default_val))
        
        mapped_values = mapping_func(token_array)
        
        # 必须转为 Float，因为后续是卷积层 (Conv1d)，它需要浮点数输入
        return torch.from_numpy(mapped_values).float()

# 实例化全局 Tokenizer
tokenizer = SignalTokenizer()

# class Load_Dataset(Dataset):
#     def __init__(self, dataset, config, training_mode):
#         super(Load_Dataset, self).__init__()
#         self.training_mode = training_mode

#         # 1. 提取数据
#         if isinstance(dataset, dict):
#             X_train = dataset["samples"]
#             y_train = dataset["labels"]
#         else:
#             X_train = dataset[0]
#             y_train = dataset[1]
            
#         # 2. 转换为 Numpy
#         if isinstance(X_train, torch.Tensor):
#             X_train = X_train.detach().cpu().numpy()
        
#         # 3. 维度修正
#         if len(X_train.shape) < 3:
#             X_train = np.expand_dims(X_train, 1)
#         if X_train.shape[1] > X_train.shape[2]:
#             X_train = np.transpose(X_train, (0, 2, 1))

#         self.x_data = X_train 
#         self.y_data = torch.as_tensor(y_train).long()
#         self.len = X_train.shape[0]

#         # 4. 数据预处理
#         # 如果是自监督训练，做增强；如果是微调/测试，直接转字母
#         if training_mode == "self_supervised":
#             self.aug1_str, self.aug2_str = DataTransform(self.x_data, config)
#         # else:
#         #     self.x_data_str = time_domain_to_letters(self.x_data)

#     def __getitem__(self, index):
#         if self.training_mode == "self_supervised":
#             str1 = self.aug1_str[index]
#             str2 = self.aug2_str[index]
            
#             # 转为 1.0, 2.0, 3.0...
#             t1 = tokenizer.convert_tokens_to_ids(str1)
#             t2 = tokenizer.convert_tokens_to_ids(str2)
            
#             return t1, self.y_data[index], t1, t2
            
#         else:
#             # 微调/测试模式
#             str_data = self.x_data_str[index]
#             token_data = tokenizer.convert_tokens_to_ids(str_data)
            
#             return token_data, self.y_data[index], token_data, token_data

#     def __len__(self):
#         return self.len
# class Load_Dataset(Dataset):
#     def __init__(self, dataset, config, training_mode):
#         super(Load_Dataset, self).__init__()
#         self.training_mode = training_mode

#         # 1. 提取数据
#         if isinstance(dataset, dict):
#             X_train = dataset["samples"]
#             y_train = dataset["labels"]
#         else:
#             X_train = dataset[0]
#             y_train = dataset[1]
            
#         # 2. 转换为 Numpy
#         if isinstance(X_train, torch.Tensor):
#             X_train = X_train.detach().cpu().numpy()
        
#         # 3. 维度修正 (Batch, Channels, Length)
#         if len(X_train.shape) < 3:
#             X_train = np.expand_dims(X_train, 1)
#         if X_train.shape[1] > X_train.shape[2]:
#             X_train = np.transpose(X_train, (0, 2, 1))

#         self.x_data = X_train 
#         self.y_data = torch.as_tensor(y_train).long()
#         self.len = X_train.shape[0]

#         # 4. 数据预处理
#         if training_mode == "self_supervised":
#             # 自监督模式：使用 DataTransform 生成增强后的两个视图 (通常带随机噪声)
#             self.aug1_str, self.aug2_str = DataTransform(self.x_data, config)
#         else:
#             # === [Fix: 微调/测试模式] ===
#             # 我们需要纯净的时域数据 和 纯净的频域数据
            
#             # (1) 时域数据 -> 转字母
#             self.x_data_time_str = time_domain_to_letters(self.x_data)
            
#             # (2) 频域数据 (FFT) -> 转字母
#             # 计算 FFT 并取幅值 (Amplitude)
#             x_freq = np.fft.fft(self.x_data, axis=2)
#             x_freq_amp = np.abs(x_freq)
#             # 使用同样的转换函数将频域幅值转为 Token
#             self.x_data_freq_str = time_domain_to_letters(x_freq_amp)

#     def __getitem__(self, index):
#         if self.training_mode == "self_supervised":
#             str1 = self.aug1_str[index]
#             str2 = self.aug2_str[index]
            
#             t1 = tokenizer.convert_tokens_to_ids(str1)
#             t2 = tokenizer.convert_tokens_to_ids(str2)
            
#             return t1, self.y_data[index], t1, t2
            
#         else:
#             # === [Fix: 微调/测试模式返回多模态数据] ===
#             # 获取时域 Token
#             str_time = self.x_data_time_str[index]
#             token_time = tokenizer.convert_tokens_to_ids(str_time)
            
#             # 获取频域 Token
#             str_freq = self.x_data_freq_str[index]
#             token_freq = tokenizer.convert_tokens_to_ids(str_freq)
            
#             # 返回: 
#             # 1. 默认输入 (通常是 time)
#             # 2. 标签
#             # 3. View 1 (Time)
#             # 4. View 2 (Freq)  <-- 这里必须填入频域数据，模型才能融合！
#             return token_time, self.y_data[index], token_time, token_freq

#     def __len__(self):
#         return self.len
# # data_generator 函数保持不变，直接复用即可
# def data_generator(data_path, configs, training_mode):

#     # --- 步骤 1: 统一加载所有需要的数据 ---
#     # 无论什么模式，我们都需要加载这三个文件来准备数据
#     print("Loading all base datasets (train, test, val)...")
#     train_dataset_full = torch.load(os.path.join(data_path, "train.pt"))
#     test_dataset_full = torch.load(os.path.join(data_path, "test.pt"))
#     valid_dataset = torch.load(os.path.join(data_path, "val.pt"))

#     # 无论哪种模式，最终用于评估的测试集都是原始的 test.pt
#     test_dataset = test_dataset_full

#     # --- 步骤 2: 根据训练模式选择数据 ---

#     if training_mode == 'self_supervised':
#         # --- 预训练模式: 合并训练集和测试集 ---
#         print("Mode: Self-supervised. Combining train and test sets for pre-training.")
#         combined_samples = torch.cat((train_dataset_full['samples'], test_dataset_full['samples']), dim=0)
#         combined_labels = torch.cat((train_dataset_full['labels'], test_dataset_full['labels']), dim=0)
        
#         train_dataset = {
#             'samples': combined_samples,
#             'labels': combined_labels
#         }
#         print(f"Total instances for pre-training: {len(combined_samples)}")
#         # 在预训练模式下，我们通常用验证集来监控，所以让 test_loader 指向验证集
#         test_dataset = valid_dataset

#     elif training_mode in ['supervised', 'fine_tune','random_init']:
#         # --- 微调/监督模式: 执行特殊的采样策略 ---
#         print(f"Mode: {training_mode}. Applying special sampling strategy.")

#         # 检查是否需要采样
#         if hasattr(configs, 'label_percentage') and configs.label_percentage < 1.0:
            
#             # 1. 确定采样数量 N (基于合并后的总数)
#             num_train_instances = len(train_dataset_full['samples'])
#             num_test_instances = len(test_dataset_full['samples'])
#             total_pool_size = num_train_instances + num_test_instances
            
#             num_samples_to_select = int(total_pool_size * configs.label_percentage)

#             print(f"Combined pool size is {total_pool_size} ({num_train_instances} train + {num_test_instances} test).")
#             print(f"Calculating {configs.label_percentage * 100:.1f}% of total pool -> selecting {num_samples_to_select} samples.")

#             # 安全检查：确保要采样的数量不超过原始训练集的大小
#             if num_samples_to_select > num_train_instances:
#                 print(f"Warning: Calculated sample count ({num_samples_to_select}) exceeds original training data size ({num_train_instances}). Capping at {num_train_instances}.")
#                 num_samples_to_select = num_train_instances

#             # 2. 确定采样来源 (从原始训练集中)
#             print(f"Selecting these {num_samples_to_select} samples FROM the original training set (size: {num_train_instances}).")
            
#             original_train_features = train_dataset_full['samples'].numpy()
#             original_train_labels = train_dataset_full['labels'].numpy()

#             # 使用 train_test_split 进行分层采样, train_size 使用计算出的绝对数量
#             features_subset, _, labels_subset, _ = train_test_split(
#                 original_train_features,
#                 original_train_labels,
#                 train_size=num_samples_to_select,
#                 stratify=original_train_labels,
#             )
            
#             print(f"Final training set size: {len(features_subset)} instances.")

#             # 将采样后的数据重新组合成字典
#             train_dataset = {
#                 'samples': torch.from_numpy(features_subset),
#                 'labels': torch.from_numpy(labels_subset)
#             }
#         else:
#             # 如果是100%数据，则直接使用全部原始训练数据
#             print("Using 100% of the original training set.")
#             train_dataset = train_dataset_full
            
#     else:
#         raise ValueError(f"Unknown training_mode: {training_mode}")

#     # --- 步骤 3: 创建 Dataset 和 DataLoader ---
#     # 这部分代码保持不变
#     train_dataset_loader = Load_Dataset(train_dataset, configs, training_mode)
#     valid_dataset_loader = Load_Dataset(valid_dataset, configs, training_mode)
#     test_dataset_loader = Load_Dataset(test_dataset, configs, training_mode)

#     train_loader = torch.utils.data.DataLoader(dataset=train_dataset_loader, batch_size=configs.batch_size,
#                                                shuffle=True, drop_last=configs.drop_last,
#                                                num_workers=0)
                                               
#     valid_loader = torch.utils.data.DataLoader(dataset=valid_dataset_loader, batch_size=configs.batch_size,
#                                                shuffle=False, drop_last=configs.drop_last,
#                                                num_workers=0)

#     test_loader = torch.utils.data.DataLoader(dataset=test_dataset_loader, batch_size=configs.batch_size,
#                                               shuffle=False, drop_last=False,
#                                               num_workers=0)

#     return train_loader, valid_loader, test_loader


import torch
import numpy as np
from torch.utils.data import Dataset

class Load_Dataset(Dataset):
    def __init__(self, dataset, config, training_mode):
        super(Load_Dataset, self).__init__()
        self.training_mode = training_mode

        # 1. 提取数据
        X_train = dataset["samples"]
        y_train = dataset["labels"]

        # 2. 维度检查与修正
        # 确保是 (Batch, Channels, Length)
        if len(X_train.shape) < 3:
            X_train = X_train.unsqueeze(2)

        # 如果通道不在第2维 (index 1)，进行 permute
        # 例如 (Batch, Length, Channels) -> (Batch, Channels, Length)
        if X_train.shape.index(min(X_train.shape)) != 1:  
            X_train = X_train.permute(0, 2, 1)

        # 3. 转 Tensor
        if isinstance(X_train, np.ndarray):
            self.x_data = torch.from_numpy(X_train).float() # 确保是 float
            self.y_data = torch.from_numpy(y_train).long()
        else:
            self.x_data = X_train.float()
            self.y_data = y_train.long()

        self.len = X_train.shape[0]

        # 注意：这里不再初始化 DataTransform，逻辑移入模型内部

    def __getitem__(self, index):
        # 无论什么模式，都只返回最原始的数据，把处理逻辑留给 Model
        return self.x_data[index], self.y_data[index]

    def __len__(self):
        return self.len
    
import os
import torch
from torch.utils.data import DataLoader
# 假设 Load_Dataset 类在别处已定义
# from .dataset import Load_Dataset 

def data_generator(data_path, configs, training_mode):

    # --- 步骤 1: 统一加载所有需要的数据 ---
    # 无论什么模式，我们都需要加载这三个文件来准备数据
    print("Loading all base datasets (train, test, val)...")
    train_dataset_full = torch.load(os.path.join(data_path, "train.pt"))
    test_dataset_full = torch.load(os.path.join(data_path, "test.pt"))
    valid_dataset = torch.load(os.path.join(data_path, "val.pt"))

    # 无论哪种模式，最终用于评估的测试集都是原始的 test.pt
    test_dataset = test_dataset_full

    # --- 步骤 2: 根据训练模式选择数据 ---

    if training_mode == 'self_supervised':
        # --- 预训练模式: 合并训练集和测试集 ---
        print("Mode: Self-supervised. Combining train and test sets for pre-training.")
        combined_samples = torch.cat((train_dataset_full['samples'], test_dataset_full['samples']), dim=0)
        combined_labels = torch.cat((train_dataset_full['labels'], test_dataset_full['labels']), dim=0)
        
        train_dataset = {
            'samples': combined_samples,
            'labels': combined_labels
        }
        print(f"Total instances for pre-training: {len(combined_samples)}")
        # 在预训练模式下，我们通常用验证集来监控，所以让 test_loader 指向验证集
        test_dataset = valid_dataset

    elif training_mode in ['supervised', 'fine_tune','random_init']:
        # --- 微调/监督模式: 执行特殊的采样策略 ---
        print(f"Mode: {training_mode}. Applying special sampling strategy.")

        # 检查是否需要采样
        if hasattr(configs, 'label_percentage') and configs.label_percentage < 1.0:
            
            # 1. 确定采样数量 N (基于合并后的总数)
            num_train_instances = len(train_dataset_full['samples'])
            num_test_instances = len(test_dataset_full['samples'])
            total_pool_size = num_train_instances + num_test_instances
            
            num_samples_to_select = int(total_pool_size * configs.label_percentage)

            print(f"Combined pool size is {total_pool_size} ({num_train_instances} train + {num_test_instances} test).")
            print(f"Calculating {configs.label_percentage * 100:.1f}% of total pool -> selecting {num_samples_to_select} samples.")

            # 安全检查：确保要采样的数量不超过原始训练集的大小
            if num_samples_to_select > num_train_instances:
                print(f"Warning: Calculated sample count ({num_samples_to_select}) exceeds original training data size ({num_train_instances}). Capping at {num_train_instances}.")
                num_samples_to_select = num_train_instances

            # 2. 确定采样来源 (从原始训练集中)
            print(f"Selecting these {num_samples_to_select} samples FROM the original training set (size: {num_train_instances}).")
            
            original_train_features = train_dataset_full['samples'].numpy()
            original_train_labels = train_dataset_full['labels'].numpy()

            # 使用 train_test_split 进行分层采样, train_size 使用计算出的绝对数量
            features_subset, _, labels_subset, _ = train_test_split(
                original_train_features,
                original_train_labels,
                train_size=num_samples_to_select,
                stratify=original_train_labels,
            )
            
            print(f"Final training set size: {len(features_subset)} instances.")

            # 将采样后的数据重新组合成字典
            train_dataset = {
                'samples': torch.from_numpy(features_subset),
                'labels': torch.from_numpy(labels_subset)
            }
        else:
            # 如果是100%数据，则直接使用全部原始训练数据
            print("Using 100% of the original training set.")
            train_dataset = train_dataset_full
            
    else:
        raise ValueError(f"Unknown training_mode: {training_mode}")

    # --- 步骤 3: 创建 Dataset 和 DataLoader ---
    # 这部分代码保持不变
    train_dataset_loader = Load_Dataset(train_dataset, configs, training_mode)
    valid_dataset_loader = Load_Dataset(valid_dataset, configs, training_mode)
    test_dataset_loader = Load_Dataset(test_dataset, configs, training_mode)

    train_loader = torch.utils.data.DataLoader(dataset=train_dataset_loader, batch_size=configs.batch_size,
                                               shuffle=True, drop_last=configs.drop_last,
                                               num_workers=0)
                                               
    valid_loader = torch.utils.data.DataLoader(dataset=valid_dataset_loader, batch_size=configs.batch_size,
                                               shuffle=False, drop_last=configs.drop_last,
                                               num_workers=0)

    test_loader = torch.utils.data.DataLoader(dataset=test_dataset_loader, batch_size=configs.batch_size,
                                              shuffle=False, drop_last=False,
                                              num_workers=0)

    return train_loader, valid_loader, test_loader







